...
async function fetchCandle(exchange, symbol, timeFrame) {
  ...
  const transformCandle = candle.reduce((accumulator, [time, open, high, low, close, volume]) => {
    accumulator.times.push(time);
    accumulator.opens.push(open);
    accumulator.highs.push(high);
    accumulator.lows.push(low);
    accumulator.closes.push(close);
    accumulator.volumes.push(volume);
    return accumulator;
  }, {
    times: [], opens: [], highs: [], lows: [], closes: [], volumes: [],
  });

  return { ...transformCandle, candleLength };
}

async function writeBuyList(trades, pair, id) {
  const { dangling = [] } = trades;
  const { buyList = _.cloneDeep(dangling) } = trades;

  buyList.push({ pair, id });
  await fs.writeJSON('./trade.json', { ...trades, buyList });
}

async function writeSellList(trades, pair, buyId, sellId = null) {
  const { dangling = [], bought = [] } = trades;
  const { buyList = _.cloneDeep(dangling), sellList = _.cloneDeep(bought) } = trades;

  _.remove(buyList, (o) => o.id === buyId);
  if (sellId !== null) {
    sellList.push({
      id: sellId, pair,
    });
  }
  await fs.writeJSON('./trade.json', { ...trades, buyList, sellList });
}
...
async function checkBuy(exchange, timeOrder, id, pair, telegram, telegramUserId) {
  ...
  if (buyFilled > 0) {
    const { price } = buyRef;
    console.log(loggingMessage(`Bought ${buyFilled} ${pair} at rate = ${price}`));
...
function checkBalance(marketName, marketBalance) {
  ...
  if (marketName === 'BTC') {
    isAboveRequiredBalance = marketBalance >= 0.0001;
...
function isAmountOk(pair, amount, rate, telegram, telegramUserId) {
  ...
  if (marketPlace === 'BTC') {
    checkAmount = (amount * rate) >= 0.0001;
...
module.exports = {
  loggingMessage, calculateMinAmount, AsyncArray, isAmountOk, messageTrade, fetchCandle, writeBuyList, writeSellList, checkBuy, checkBalance, commonIndicator, upTrend, calculateAmount2Sell, smoothedHeikin, slowHeikin, restart,
};
